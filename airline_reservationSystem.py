...
last_first_class_assigned = 0
last_economy_assigned = 5

while True:
    print()
    
    if last_first_class_assigned == 5 and last_economy_assigned == 10:
        print("ALL seats filled!")
        break
    
    seat_type = input('Press 1 for first class, 2 for economy: ')
    seat_type = int(seat_type)

    if seat_type == 1:
        if last_first_class_assigned == 5:
            if last_economy_assigned == 10:
                print('Next Flight leave in 3 hrs')

            else:
                ask_economy = input('Would you like Economy instead. Type y for yes, n for no: ')

                if ask_economy.lower() == 'y':
                    seats[last_economy_assigned] = True
                    last_economy_assigned += 1
            ...
            seats[last_first_class_assigned] = True
            last_first_class_assigned += 1
        ...
        if last_economy_assigned == 10:
            if last_first_class_assigned == 5:
                ...
                ask_first_class = input('Would you like First Class instead?. Type y for yes, n for no: ')

                if ask_first_class.lower() == 'y':
                    seats[last_first_class_assigned] = True
                    last_first_class_assigned += 1
